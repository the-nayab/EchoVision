...
window.addEventListener("load", () => {
  audioCheck();
  stopSampleAudio();  // Ensure sample audio is stopped on load
});
...
function stopSampleAudio() {
  // Get the audio element directly using its source file
  const audioElement = document.querySelector('audio[src="public/audio/sample.mp3"]');
  if (audioElement) {
    audioElement.pause();
